class Frontend::SearchController < Frontend::BaseController
  def search_result
    if params[:search][:keywords].presence
      ...
      @search_products_results = build_query_filter(Product.where("features ->> 'is_shelves' = ?", '1').where(site: @cms_site.site.self_and_descendants).or(Product.where(site: @cms_site.site.parent)).all, only: @filter_products_colums).order(updated_at: :desc)
