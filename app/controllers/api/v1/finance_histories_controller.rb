...
class Api::V1::FinanceHistoriesController < Api::V1::BaseController
  ...
  def index
    ...
    if ['in', 'out'].include?(operate_type) 
      ...
      orders = Order.where("code like :key", {key: ['%',params['order_code'].upcase, '%'].join}) if params[:order_code]
      ...
      render json: {
        finance_histories: operate_type == 'in' ? order_finance_json(finance_histories) : finance_json(finance_histories),
        page_size: page_size,
  ...
  def finance_json(finance_histories)
    finance_histories.as_json(only: [:id, :operate_type, :operate_date, :amount, :note])
  end
