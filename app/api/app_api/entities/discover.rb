    ...
    class Discover < Base
      ...
      def resource_data
        if object.resource.class.to_s == 'Product'
          ...
          object.resource.as_json(only: [:id, :title, :description, :created_at], include: { user: { only: [:nickname], methods: [:display_headshot] }, image_items: { only: [], methods: [:image_url] }, products: { only: [:id, :name], methods: [:first_image] } }).merge(discover_type: 'article')
        end
      end
    end
  end
