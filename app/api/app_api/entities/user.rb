module AppAPI
  module Entities
    class User < Base
      ...
      expose :description, documentation: { desc: '收藏的店铺数量，特定条件下才能获得' } do |user, options|
        options[:site_favorites_count]
      end

      expose :site_favorites_count, documentation: { desc: '收藏的店铺数量，特定条件下才能获得' } do |user, options|
        options[:site_favorites_count]
      end

      expose :product_favorites_count, documentation: { desc: '收藏的产品数量，特定条件下才能获得' } do |user, options|
        options[:product_favorites_count]
      end

      # TODO: 分享的帖子数量，特定条件下才能获得
      expose :article_shares_count, documentation: { desc: '分享的帖子数量，特定条件下才能获得（功能未实现，始终返回0）' } do |user, options|
        options[:article_shares_count]
      end
