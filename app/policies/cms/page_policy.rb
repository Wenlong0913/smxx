class Cms::PagePolicy < ApplicationPolicy
  ...
  def permitted_attributes_for_create
    if user.super_admin_or_admin?  || user.has_role?(:agent)
      [:channel_id, :title, :short_title, :keywords, :description, :image_path, :content, :forage_url, :is_foraged, :forage_from, :forage_district_from, :forage_image, :tag_list => [], :properties => []]
    else
      []
    end
  end
