class ProductPolicy < ApplicationPolicy
  ...
  def permitted_attributes_for_create
    if user.super_admin_or_admin? || user.has_role?(:agent)
      [:site_id, :name, :price, :stock, :unit, :discount, :description, :content, :weight, :weight_unit, :additional_attribute_keys, :additional_attribute_values, :is_shelves, :is_fee, :shopping_fee, :catalog_id, :hot, :recommend, :event, :promotion, :discount, :image_item_ids => []]
    else
