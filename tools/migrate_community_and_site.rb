...
class MigrateCommunityAndSite
  ...
  def migrate_community(import_info)
    begin
      c = Community.find_or_initialize_by(name: import_info.name)
      ...
      address_arr = []
      address_arr << c.province unless c.address_str.include?(c.province)
      address_arr << c.city unless c.address_str.include?(c.city)
      address_arr << c.district unless c.address_str.include?(c.district)
      address_arr << c.address_str
      address_arr << c.name unless c.address_str.include?(c.name)
      c.address_line = address_arr.join.strip
      c.save!
  ...
  def migrate_site(import_info)
    begin
      c = Site.find_or_initialize_by(title: import_info.name)
      failed_data = ImportFailedInformation.find_or_initialize_by(origin_type: import_info.origin_type, file_name: import_info.file_name, line: import_info.line)
      failed_data.name        = import_info.name
      ...
      c.address_line    = [c.province, c.real_city, c.city, c.district, import_info.address_str.gsub(/(?:\(|（).*(?:\)|）)$/, ''), c.title].join.strip
      c.save!
      import_info.is_parsed = 'y'
    rescue
      failed_data.save!
