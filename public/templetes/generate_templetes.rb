#encoding: utf-8
# 教程参考： readme.txt
# 控制台下运行该文件： ruby generate_templetes.rb -e <模板目录名>
...
class DataExtractor
  ...
  def run
    ...
    head_path = File.join(File.dirname(__FILE__), @theme, '_head.html.erb')
    header_path = File.join(File.dirname(__FILE__), @theme, '_header.html.erb')
    foot_path = File.join(File.dirname(__FILE__), @theme, '_foot.html.erb')
    footer_path = File.join(File.dirname(__FILE__), @theme, '_footer.html.erb')
    temp_index_path = File.join(File.dirname(__FILE__), @theme, 'temp_index.html.erb')
    temp_detail_path = File.join(File.dirname(__FILE__), @theme, 'temp_detail.html.erb')
    ...
    if /<\/head>(.*)<\/html>/im =~ index_content.force_encoding("utf-8")
      ...
      File.open(temp_detail_path, 'w'){|f| f.write( get_content(the_content) )}
    end
    ...
    temp_list.each do |t|
      ...
      if /<body[^>]*>(.*)<\/body>/im =~ the_content.force_encoding("utf-8")
        the_content = $1
        %W[header footer].each do |s|
          the_content = the_content.sub(/<!--\s*\[\[#{s} start\]\]\s*-->(.*)<!--\s*\[\[#{s} end\]\]\s*-->/im, "<%= render file: 'public/templetes/#{@theme}/_#{s}' %>")
        end
  ...
  def get_content(content)
    ...
    content.gsub!(/ src\s*=\s*('|")(assets|img|images|image|js|javascript|javascripts|css|font|fonts|ico|icos|icon|icons)\//, ' src=\1/templetes/{{theme}}/\2/')
    content.gsub!(/ href\s*=\s*('|")(assets|img|images|image|js|javascript|javascripts|css|font|fonts|icos|icons|ico|icon)\//, ' href=\1/templetes/{{theme}}/\2/')
    #css
